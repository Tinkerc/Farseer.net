...
namespace FS.Utils.Web.Common
{
    ...
    {
        private static readonly Cache _webCache = HttpRuntime.Cache;
        ...
        public static void Add<T>(string key, T t, int timeOut = 1440)
        {
            ...
            {
                _webCache.Insert(key, t, null, DateTime.Now.AddMinutes(timeOut), Cache.NoSlidingExpiration);
            }
        }
        ...
        {
            return string.IsNullOrEmpty(key) ? default(T) : _webCache.Get(key).ConvertType<T>();
        }
